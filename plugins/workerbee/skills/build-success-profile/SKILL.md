...
| `update_capability_role` | Refine the standard from manager context or top performers. Params: `jobRoleId`, `skills: [{ skillName, importance }]` where importance ∈ `non-negotiable` \| `core` \| `nice-to-have` \| `exclude`. `non-negotiable` is a **hard gate** — a candidate missing it is penalized in ranking (its score is roughly halved); reserve it for true deal-breakers ("must absolutely have X", "don't show me anyone without X") and use `core` for ordinary required skills. |
